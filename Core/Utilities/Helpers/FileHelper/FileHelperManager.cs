...
namespace Core.Utilities.Helpers.FileHelper
{
    public class FileHelperManager : IFileHelper
    {
        public void Delete(string filePath)
        {
            ...
            {
                File.Delete(filePath); // Eğer varsa siliyor.
            }
        }
        ...
        public string Upload(IFormFile file, string root)
        {
            if (file.Length > 0)
            {
                if (!Directory.Exists(root)) // Eğer directory diskte yoksa o directoryi oluşturur.
                {
                    Directory.CreateDirectory(root);
                }
                string extension = Path.GetExtension(file.FileName); // Dosyanın uzantısını aldık
